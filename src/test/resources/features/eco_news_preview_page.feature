Feature: Preview News Before Publishing
  ...
  Scenario: User previews news content
    ...
    And the preview should display the title "Test Preview"
    And the preview should display the tag "News"
    And the preview should display the content text "This is a test preview content"
  ...
  Scenario: User publishes eco news successfully
    ...
    Then the eco news should be successfully published
