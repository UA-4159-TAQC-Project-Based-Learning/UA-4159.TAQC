...
public class CreateNewsTagSelectionStep {
    ...
    public void the_user_clicks_the_publish_button() {
        ecoNewsPage = createNewsPage.getCreateNewsButtonsComponent().clickPublish();
    }

    @Then("the news should be published successfully")
    public void the_news_should_be_published_successfully() {
        hooks.getSoftAssert().assertNotNull(ecoNewsPage, "After clicking Publish user should be redirected to EcoNewsPage");

        this.ecoNewsDetailsPage = ecoNewsPage.getOneTableCardByTitle(enteredTitle).goToDetails();

        hooks.getSoftAssert().assertEquals(ecoNewsDetailsPage.getTitle(), enteredTitle, "Published news title does not match expected");
    }
    ...
    public void only_3_tags_should_remain_selected(String tags) {
        ...
        hooks.getSoftAssert().assertEquals(actual.size(), 3, "User should not be able to select more than 3 tags");

        hooks.getSoftAssert().assertEquals(actual, expected, "Selected tags do not match expected after selecting more than 3");
    }
    ...
    public void the_selected_tags_should_not_include_tags(String tags) {
        ...
        for (String tag : parseTags(tags)) {
            hooks.getSoftAssert().assertFalse(actual.contains(tag), "Unexpected tag '" + tag + "' is selected");
        }
    }
